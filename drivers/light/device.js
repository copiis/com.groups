...
class Device extends Homey.Device {
  ...
  /**
   * Update the value of all the devices
   *
  ...
  async updateCapability(valueObj, optsObj) {
    this.log('Update Capability Group ' + this.getName());

    // Loop through each devices in the group
    for (let x in this.settings.devices) {

      // Get the WebAPI reference device ::
      let device = await this.getDevice(this.settings.devices[x]);

      // Using the API for the device, set the capability value, to what ever we just changed.
  ...
  /**
   * Will initialise the states by adding an event when ever a device state (capability) is changed.
  ...
  async initEvents() {
    ...
    for (let x in this.settings.devices) {
      ...
      if (this.settings.devices.hasOwnProperty(x)) {
        ...
        // Store our events, so we can remove them if needed.
        this.events[this.settings.devices[x]] = async (state) => { this.onStateChange(state, this.settings.devices[x]); }
        ...
        device.on('device.update' , this.events[this.settings.devices[x]]);
        //device.on('$update' , this.events[this.settings.devices[x]]);
      }
    }
  }
  ...
  async getDevicesCapabilityValues(capabilities) {
    ...
    for (let x in this.settings.devices) {
      ...
      if (this.settings.devices.hasOwnProperty(x)) {
        ...
        for (let i in capabilities) {
          values[capabilities[i]].push(this.states[this.settings.devices[x]][capabilities[i]]);
        }
      }
    }
  ...
  async destroyEvents() {
    ...
    if (this.hasOwnProperty('events')) {
      ...
      // Loop ALL devices
      for (let x in this.settings.devices) {
        if (this.events.hasOwnProperty(this.settings.devices[x])) {
          let device = await this.getDevice(this.settings.devices[x]);
          // @todo this should also work, so long as the event is named correctly.
          await device.removeListener('$update', this.events[this.settings.devices[x]]);
          this.events[this.settings.devices[x]] = null; // @todo GC should handle this
        }
      }
    }
    return true;
