...
class Driver extends Homey.Driver {
    ...
    onDevicesChanged (data, callback) {
        ...
        for (let i in data.devices) {
            ...
            // After a GIGO check, add the devices icon to our list of icons to display.
            if (data.devices[i].hasOwnProperty('iconObj') && data.devices[i].iconObj.hasOwnProperty('url')) {
                ...
                this.icons[data.devices[i].iconObj.url] = this.appRoot +'/assets/icon.svg';//data.devices[i].iconObj.url;
            }
        }

        this.group.settings.devices = ids;
        callback(null, this.group);
    }
