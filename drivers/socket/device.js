...
class Device extends Homey.Device {
    ...
    for (let x in this.settings.groupedDevices) {
      ...
      if (this.settings.groupedDevices.hasOwnProperty(x)) {
        ...
        for (let i in this.capabilities) {
          ...
          // Store our events, so we can remove them if needed.
          this.events[this.settings.groupedDevices[x]][this.capabilities[i]] = async (value) => { this.onStateChange(value, this.capabilities[i], this.settings.groupedDevices[x]); }

          // Link our event to a state change of the capability.
          // @todo investigate doing this through the app - to reduce event watchers.
          device.makeCapabilityInstance(this.capabilities[i], this.events[this.settings.groupedDevices[x]][this.capabilities[i]]);
        }
      }
    }
  }
   ...
   * @returns {Promise<void>}
   */
  async onStateChange(value, capability, device) {

    this.log('State Change '+ this.getName() + ' ' + capability + ' ' + state);

    // Update values to the latest ones we have, prior to using the value to set capability values
    this.states[device][capability] = value;
    ...
    for (let x in this.settings.groupedDevices) {
      ...
      if (this.settings.groupedDevices.hasOwnProperty(x)) {
        ...
        // Loop through each of the capabilities checking each of the devices value.
          values[capability].push(this.states[this.settings.groupedDevices[x]][capability]);
      }
    }
