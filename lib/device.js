...
class Device extends Homey.Device {
	 ...
	 */
	async initValues() {
	    this.log('<initValues[' + this.getName() + ']>'); ;
    ...
    async initGroupListener() {
	    ...
	    this.log('<initGroupListener[' + this.getName() + ']>'); ;
        ...
        return this.registerMultipleCapabilityListener(this.capabilities, async (values, options) => {
	        this.log('<registerMultipleCapabilityListener[' + this.getName() + ']:>' + JSON.stringify(values) + '</registerMultipleCapabilityListener>');
    ...
    async updateDevicesCapabilities(values) {
	    this.log('<updateDevicesCapabilities[' + this.getName() + ']/>');
                    ...
                    setTimeout( () => {
                        try {
                            a.instance.setValue(values[capability]).then(() => {
	                            this.log('<updateDevicesCapabilities[' + this.getName() + ']>Set ' + a.instance.title + ' for ' + a.instance.device.name + '(' + values[capability] + ')</updateDevicesCapabilities>')
                            }).catch((error) => {

	                            this.log('<setValue:ERROR:' + error.code + '>' + error.name + '</setValue:ERROR:>');

                                a.instance.setValue(values[capability]).then( () => {
                                    this.log('<updateDevicesCapabilities:RETRY[' + this.getName() + ']>Set ' + a.instance.title + ' for ' + a.instance.device.name + '(' + values[capability] + ')</updateDevicesCapabilities:RETRY>')
                                }).catch(() => {
                                    ...
                                    this.log('<updateDevicesCapabilities:FAILED[' + this.getName() + ']>Set ' + a.instance.title + ' for ' + a.instance.device.name + '(' + values[capability] + ')</updateDevicesCapabilities:FAILED>');
                                });
        ...
        Homey.app.getApi().then((api) => {
            ...
            api.devices.on('device.delete', async (device) => {
                ...
                if (this.settings.devices.includes(device.id)) {
                    ...
                    this.log('<audit['+ this.getName() +']:' +device.id+': Detected device in group has been deleted>');
    ...
    async destroyDevice(deviceId) {
        ...
        this.log('<destroyDevice[' + this.getName() + '] destroying  ' + deviceId +'>');
    ...
    async destroyDeviceAudit(deviceId) {
        this.log('<destroyDeviceAudit[' + this.getName() + '] clearing audit  ' + deviceId +'>');
    ...
    async destroyDeviceInstances(deviceId) {
        ...
        this.log('<destroyDeviceInstances[' + this.getName() + '] clearing instances  ' + deviceId +'>');
