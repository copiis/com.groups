...
class Device extends Homey.Device {
    ...
    async validateDeviceInstances(deviceId) {
        ...
        if (device) {
            ...
            {
                //this.log('<validateDeviceInstances['+ this.getName() +']:' +deviceId+': No change>');
            }

        } else {

            // We don't have a device, log then throw an error which will get caught by auditDevice.
            this.log('<validateDeviceInstances['+ this.getName() +']:' +deviceId+': Removing (deleted) device instances ('+device.name+')>');
    ...
    async auditDevice(deviceId) {
        ...
        try {
            ...
            if (this.auditTimer) {
                ...
                // DEBUG : this.audits[deviceId] = setTimeout(this.auditDevice.bind(this,deviceId), 3000);
    ...
    async audit() {
        ...
        Homey.app.getApi().then((api) => {
            ...
            api.devices.on('device.delete', async (device) => {
                ...
                if (this.settings.devices.hasOwnProperty(device.id)) {

                    // Gigo - we could just delete it, but run through the process, just in case.
                    try {
                        this.validateDeviceInstances(device.id);
                    } catch (e) {

                        // ValidateDeviceIsntance throws an error if the device no longer exists.
                        if (e.name === 'device_not_found') {
