...
class Device extends Homey.Device {
    ...
    async setGroupCapabilityValue(capability, values) {
        ...
        if (a.method !== false && a.method !== 'ignore') {
            ...
            a.fml = Helper[Homey.app.library.getMethod(a.method).function](a.fml);

            // Convert the value in the to capabilities required type
            a.fml = Helper[a.capability.type](a.fml);
            ...
            try {
                ...
                await this.setCapabilityValue(a.key, a.fml);
                console.log('Setting group capability value : ' + a.key + ' to ' + a.fml + ' :: [' + values.join(', ') + '](' + a.method + ')'); // DEBUG
            } catch (error) {
                console.log('Error on setting capability value : ' + a.key + ' ' + a.fml + ' Error:' + error.message); // DEBUG
