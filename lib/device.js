 ...
 *
 * @todo : seems like the delete  action is called mutliple times, more likely the delete action is called upon a group it should be called upon.
 *
 * In this case the[heater] was deleted but then an audit runs saying that a device in the group shas been deleted?
 *
 device.delete: 753e682d-f790-4f64-be14-7762b38630cd
 2019-01-22 02:57:30 [log] [ManagerDrivers] [fan] [0] <getDevice[Fan < group >]:753e682d-f790-4f64-be14-7762b38630cd: device no longer exists.>
 2019-01-22 02:57:30 [log] [ManagerDrivers] [fan] [0] <validateDeviceInstances[Fan < group >]:753e682d-f790-4f64-be14-7762b38630cd: Removing (deleted) device instances>
 2019-01-22 02:57:30 [log] [ManagerDrivers] [heater] [0] <getDevice[Heater < group >]:753e682d-f790-4f64-be14-7762b38630cd: device no longer exists.>
 2019-01-22 02:57:30 [log] [ManagerDrivers] [heater] [0] <validateDeviceInstances[Heater < group >]:753e682d-f790-4f64-be14-7762b38630cd: Removing (deleted) device instances>
 2019-01-22 02:57:30 [log] [ManagerDrivers] [heater] [0] <getDevice[Heater < group >]:753e682d-f790-4f64-be14-7762b38630cd: device no longer exists.>
 2019-01-22 02:57:30 [log] [ManagerDrivers] [heater] [0] <validateDeviceInstances[Heater < group >]:753e682d-f790-4f64-be14-7762b38630cd: Removing (deleted) device instances>
 *
 * @todo seems like there is an issue with the auditDevice timing, where it runs much to frequently.
 * ie. every 5 second on error : 30 second on success?
 *
 * @todo seems to be an issue where once an device has been deleted the audit is not being cleared properly, then it attempts to recreate deleted item.
 *
 * Likely code needs to be added which will delete all instances/audits when the device itself is deleted.
 *
 2019-01-22 02:58:14 [log] [ManagerDrivers] [heater] [0] <validateDeviceInstances[Heater < group >]:8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f: Creating device instances (Mirror Lights) on 1 capabilities>
 2019-01-22 02:58:14 [err] [ManagerDrivers] [heater] [0] validateDeviceInstances: error setting 8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f capability onoff for Mirror Lights. Is deivce is ready? : true. Does the instance exist undefined
 2019-01-22 02:58:19 [log] [ManagerDrivers] [heater] [0] <validateDeviceInstances[Heater < group >]:8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f: Creating device instances (Mirror Lights) on 1 capabilities>
 2019-01-22 02:58:19 [err] [ManagerDrivers] [heater] [0] validateDeviceInstances: error setting 8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f capability onoff for Mirror Lights. Is deivce is ready? : true. Does the instance exist undefined
 2019-01-22 02:58:24 [log] [ManagerDrivers] [heater] [0] <validateDeviceInstances[Heater < group >]:8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f: Creating device instances (Mirror Lights) on 1 capabilities>
 2019-01-22 02:58:24 [err] [ManagerDrivers] [heater] [0] validateDeviceInstances: error setting 8a4daa2a-80a6-4e8d-8cd2-9d08f00d672f capability onoff for Mirror Lights. Is deivce is ready? : true. Does the instance exist undefined
 *
 */
class Device extends Homey.Device {
    ...
    async validateDeviceInstances(deviceId) {
        ...
        if (device) {
            ...
            exists = (this.instances[device.id]) && (Object.getOwnPropertyNames(this.instances[device.id]).length > 0);
            ...
            if (ready !== exists) {
                ...
                if (exists) {
                    ...
                    this.log('<validateDeviceInstances['+ this.getName() +']:' +deviceId+': Removing (unavailable) device instances ('+device.name+')>');
                    ...
                    // if doesnt exists - there for ready.
            ...
            this.log('<validateDeviceInstances['+ this.getName() +']:' +deviceId+': Removing (deleted) device instances>');
    ...
    async audit() {
        ...
        Homey.app.getApi().then((api) => {
            ...
            api.devices.on('device.delete', async (device) => {
                ...
                if (this.settings.devices.indexOf(device.id)) {
    ...
    async setGroupCapabilityValue(capability, values) {
        ...
        if (a.method !== false && a.method !== 'ignore') {
            ...
            try {
                ...
                // Log for debugging.
                this.log('<setGroupCapabilityValue['+this.getName()+']: Setting group capability : ' + a.key + ' to ' + a.value + ' :: [' + values.join(', ') + '](' + a.method + ')');
            } catch (error) {
    ...
    async getDevice(deviceId) {
        ...
        try {
            ...
            if (e.name === 'device_not_found') {
                this.log('<getDevice['+ this.getName() +']:' +deviceId+': device no longer exists.>');
            }
            return false;
        }
    }
    ...
    async destroyAllAudits() {
        ...
        for (let a in this.audits) {
            await this.destroyDeviceAudit(this.audits[a])
        }
